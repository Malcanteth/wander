...
function GetDungeonCool : byte;       // Вернуть крутизну пещеры
...
begin
  ...
  for x:=1 to MapX do
    for y:=1 to MapY do
      M.Tile[x,y] := tdEWALL;
  // Пол
  for x:=10 to MapX-10 do
    for y:=5 to MapY-5 do
      begin
        M.Tile[x,y] := tdEARTH;
        if Random(4)+1 = 1 then M.Blood[x,y] := Random(2)+1;
      end;
  // Тараканы и крысы
  for x:=11 to MapX-11 do
    ...
    begin
      ...
      CreateMonster(i, x, 6 + Random(2));
      CreateMonster(i, x, MapY-6-Random(2));
    end;
  for y:=6 to MapY-6 do
    if Random(3)+1 = 1 then
    begin
      case Random(2)+1 of
        1 : i := mdCOCKROACH;
      ...
      CreateMonster(i, 11 + Random(2), y);
      CreateMonster(i, MapX-11 - Random(2), y);
    end;
  // Лестница
  M.Tile[40,10] := tdUSTAIRS;
  // Поместить героя на лестницу вверх
  pc.PlaceAtTile(tdUSTAIRS);
end;
...
begin
  ...
  pc.level := 255;
  // Красный пол
  for x:=2 to MapX-1 do
    for y:=2 to MapY-1 do
      begin
        if Random(3)+1 = 3 then
...
begin
  ...
  begin
    ...
    if (pc.x = MapX-3) and (pc.y = 5) then
      Result := 1;
    // Погреб
    if (pc.x = 66) and (pc.y = 27) then
      Result := 2;
  end;
end;

{ Вернуть крутизну пещеры }
function GetDungeonCool : byte;
begin
  Result := 0;
  // Эвилиар
  if pc.level = 1 then
