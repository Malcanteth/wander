...
var
  s1, s2: string;
begin
  s1 := Format('Выбери атрибут, в котором %s больше всего преуспел{/a}:', [pc.name]);
  // 'Выбери атрибут, в котором '+pc.name+' больше всего преуспел{/a}:';
  s2 := Format('А теперь выбери атрибут, которому %s тоже уделял{/a} внимание:', [pc.name]);
  // 'А теперь выбери атрибут, которому '+pc.name+' тоже уделял{/a} внимание:';
  StartDecorating('<-СОЗДАНИЕ НОВОГО ПЕРСОНАЖА->', TRUE);
  with GScreen.Canvas do
  begin
    Font.Color := cWHITE;
    ...
    for i := 1 to CLOSEFIGHTAMOUNT - 1 do
      if wlist[i] > 0 then
        ...
        begin
          ...
          case wlist[i] of
            2:
              TextOut(44 * CharX, (14 + i) * CharY, 'Меч');
            3:
              TextOut(44 * CharX, (14 + i) * CharY, 'Дубина');
            4:
              TextOut(44 * CharX, (14 + i) * CharY, 'Посох');
            5:
              TextOut(44 * CharX, (14 + i) * CharY, 'Топор');
            6:
              TextOut(44 * CharX, (14 + i) * CharY, 'Рукопашный бой');
          end;
        end;
    Font.Color := cYELLOW;
    TextOut(41 * CharX, (14 + MenuSelected) * CharY, '>');
    ...
    for i := 1 to FARFIGHTAMOUNT do
      if wlist[i] > 0 then
        ...
        begin
          ...
          case wlist[i] of
            2:
              TextOut(44 * CharX, (14 + i) * CharY, 'Лук');
            3:
              TextOut(44 * CharX, (14 + i) * CharY, 'Праща');
            4:
              TextOut(44 * CharX, (14 + i) * CharY, 'Духовая трубка');
            5:
              TextOut(44 * CharX, (14 + i) * CharY, 'Арбалет');
          end;
        end;
    Font.Color := cYELLOW;
    TextOut(41 * CharX, (14 + MenuSelected) * CharY, '>');
