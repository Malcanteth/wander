...
const
  ...
  AbilitysData: array [1 .. AbilitysAmount] of TAbility = ((name: 'Быстрое насыщение'; descr: 'Большее насыщение от того же рациона'; koef: 20;
    ), (name: 'Зоркость'; descr: 'Ты видишь дальше чем обычно'; koef: 1;
    ), (name: 'Молниеносная реакция'; descr: 'Повышенный шанс провести контратаку'; koef: 2;
    ), (name: 'Повышенная регенерация'; descr: 'Твои раны затягиваются гораздо быстрее, чем раньше'; koef: 0.5;
    ), (name: 'Меткость'; descr: 'Больший шанс попасть по противнику во время атаки'; koef: 1;
    ), (name: 'Уклонение'; descr: 'Ты чаще уклоняешься от атак противника'; koef: 1;
    ), (name: 'Быстрая атака'; descr: 'Тебе чаще удается ударить противника дважды за ход'; koef: 1;
    ), (name: 'Энергичность'; descr: 'Повышенная скорость выполнения любых действий'; koef: 5;
    ), (name: 'Внимательность'; descr: 'Ты быстрее находишь секретные двери'; koef: 1;));
  ...
  CLOSEWPNNAME: array [1 .. CLOSEFIGHTAMOUNT] of string = ('Двуручное оружие', 'Клинки', 'Дубины', 'Жезлы и посохи', 'Топоры', 'Рукопашный бой');
  FARWPNNAME: array [1 .. FARFIGHTAMOUNT] of string = ('Кинуть/швырнуть', 'Луки', 'Пращи', 'Духовые трубки', 'Арбалеты');
  ARMORTYPENAME: array [1 .. ARMORTYPEAMOUNT] of string = ('Одежда', 'Легкая броня', 'Тяжелая броня');
    ...
    begin
      for i := 1 to CLOSEFIGHTAMOUNT do
        ...
        begin
          ...
          if ShowProc then
            TextOut(33 * CharX, (top + i) * CharY, RateToStr(RateSkill(pc.closefight[i])) + ' = ' + FloatToStr(pc.closefight[i]) + '%')
          else
    ...
    begin
      for i := 1 to FARFIGHTAMOUNT do
        ...
        begin
          ...
          if ShowProc then
            TextOut(33 * CharX, ((top + 10) + i) * CharY, RateToStr(RateSkill(pc.farfight[i])) + ' = ' + FloatToStr(pc.farfight[i]) + '%')
          else
    ...
    begin
      for i := 1 to MAGICSCHOOLAMOUNT do
        ...
        begin
          ...
          Font.Color := ColorRateSkill(pc.farfight[i]);
          if ShowProc then
            TextOut(33 * CharX, ((top + 20) + i) * CharY, RateToStr(RateSkill(pc.farfight[i])) + ' = ' + FloatToStr(pc.farfight[i]) + '%')
          else
...
function IsInNewAreaSkill(was, now: real): boolean;
begin
  Result := FALSE;
  if RateSkill(was) <> RateSkill(now) then
    Result := TRUE;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Font.Color := cWHITE;
      TextOut((((85 - length(AbilitysData[FullAbilitys[MenuSelected]].descr)) div 2) + 8) * CharX, 38 * CharY,
        AbilitysData[FullAbilitys[MenuSelected]].descr);
    end
    else
