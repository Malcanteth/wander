...
type
  Tpc = object (TMonster)
    ...
    //
    status      : array[1..1] of integer;        // Счетчики (1-Голод)
    ...
    procedure DeleteInvItem(n : byte;
...
begin
  ...
  PickUp(CreateItem(idCOIN, 40), FALSE);
...
begin
  ...
  begin
    ...
    begin
      ...
      if pc.depth = 0 then PlaceOnStairs(pc.level, pc.enter);
      pc.turn := 2;
      if pc.depth > 0 then
      ...
      begin
        ...
        if f = false then
          for i:=1 to MaxHandle do
            ...
            begin
              if (pc.invmass + ItemsData[Item.id].mass < pc.MaxMass) or (FromEq) then
              begin
                pc.Inv[i] := Item;
                pc.invmass := pc.invmass + ItemsData[Item.id].mass;
                break;
              end else
                begin
                  Result := 3;
...
procedure TPc.DeleteInvItem(n : byte; full : boolean);
begin
  if (pc.inv[n].amount > 1) and (not full) then
