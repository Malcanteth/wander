     ...
     for y:=1 to MapY do
       if M.Tile[x,y] = tdFLOOR then
         ...
         begin
           ...
           until
             (not IsFlag(MonstersData[i].flags, M_NEUTRAL)) and ((pc.cool >= MonstersData[i].coollevel) or ((pc.cool = MonstersData[i].coollevel+1) and (Random(10)+1=10)));
           CreateMonster(i,x,y);
         end;
  end;
  // Поместить предметы
      ...
      for y:=1 to MapY do
        if (M.Tile[x,y] = tdFLOOR) and (Random(200)+1 = 1) then
          case Random(8)+1 of
            ...
            8 :
            case Random(100)+1 of
              1..70   : PutItem(x,y,CreateItem(idPOTIONCURE, 1, 0));
              71..100 : PutItem(x,y,CreateItem(idPOTIONHEAL, 1, 0));
            end;
          end;
  end;
  begin
