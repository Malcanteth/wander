...
begin
  ...
  begin
      ...
      case id of
        ...
        begin
          w := False;
          if (Ask(MonstersData[id].name1 + ' говорит: "Хочешь купить кусок отличного свежего мяса всего за 25 золотых?" [(Y/n)]')) then
          begin
            if pc.FindCoins = 0 then
              AddMsg('К сожалению, у тебя совсем нет денег.') else
              if pc.inv[pc.FindCoins].amount < 25 then
                AddMsg('У тебя недостаточно золотых монет для покупки.') else
                if pc.inv[pc.FindCoins].amount >= 25 then
                begin
                  AddMsg('Ты протягиваешь '+MonstersData[id].name3+' деньги.');
                  dec(pc.inv[pc.FindCoins].amount, 25);
                  More;
