...
begin
  ...
  // Выводим
  case GameState of
    gsPLAY, gsCLOSE, gsLOOK, gsTALK, gsATACK:
    begin
      // Выводим карту
      M.DrawScene;
      // Выводим сообщения
      ...
      begin
        ...
        begin
          ...
          case GameState of
            ...
            begin
              case Key of
                35,97,49  : pc.Fight(M.MonL[M.MonP[pc.x-1,pc.y+1]]);
                40,98,50  : pc.Fight(M.MonL[M.MonP[pc.x,pc.y+1]]);
                34,99,51  : pc.Fight(M.MonL[M.MonP[pc.x+1,pc.y+1]]);
                37,100,52 : pc.Fight(M.MonL[M.MonP[pc.x-1,pc.y]]);
                39,102,54 : pc.Fight(M.MonL[M.MonP[pc.x+1,pc.y]]);
                36,103,55 : pc.Fight(M.MonL[M.MonP[pc.x-1,pc.y-1]]);
                38,104,56 : pc.Fight(M.MonL[M.MonP[pc.x,pc.y-1]]);
                33,105,57 : pc.Fight(M.MonL[M.MonP[pc.x+1,pc.y-1]]);
                else
                  AddDrawMsg('Указано неправильное направление!');
              end;
              pc.turn := 1;
            ...
            begin
              case Key of
                35,97,49  : pc.Talk(-1,1);
                40,98,50  : pc.Talk(0,1);
                34,99,51  : pc.Talk(1,1);
                37,100,52 : pc.Talk(-1,0);
                39,102,54 : pc.Talk(1,0);
                36,103,55 : pc.Talk(-1,-1);
                38,104,56 : pc.Talk(0,-1);
                33,105,57 : pc.Talk(1,-1);
                else
                  AddDrawMsg('Указано неправильное направление!');
              end;
              pc.turn := 1;
                              ...
                              case ItemsData[pc.Inv[MenuSelected].id].vid of
                                ...
                                begin
                                  case pc.EquipItem(pc.Inv[MenuSelected]) of
                                    ...
                                    1 :
                                    begin
                                      AddMsg('Ты уже используешь другой предмет!');
                                    end;
                                  end;
                                end;
                                // Съесть
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    begin
                                      AddMsg('[Ты не смог доесть '+ItemsData[pc.Inv[MenuSelected].id].name3+', потому что очень насытился... через чур насытился...]');
                                      pc.status[stHUNGRY] := -500;
                                    end else
                                        AddMsg('[Ты съел '+ItemsData[pc.Inv[MenuSelected].id].name3+'.]');
                                    pc.DeleteInvItem(MenuSelected, FALSE);
                                    pc.turn := 1;
                                  end else
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    begin
                                      inc(pc.hp, Random(10)+1);
                                      if pc.Hp >= pc.RHp then
                                      begin
                                        pc.Hp := pc.RHp;
                                        AddMsg('[Ты полностью исцелился!]');
                                      end else
                                        AddMsg('[Тебе стало немного лучше.]');
                                    end else
...
begin
  ...
  begin
    ...
    if (GameState = gsPLAY) or (GameState = gsLOOK) or (GameState = gsCLOSE) or (GameState = gsTALK)  then
    begin
