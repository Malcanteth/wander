      ...
      for a := pc.x - 1 to pc.x + 1 do
        for b := pc.y - 1 to pc.y + 1 do
          if (a>0)and(a<=MapX)and(b>0)and(b<=MapY) then
            ...
            begin
              case whattodo of
                1 : Fight(M.MonL[M.MonP[a,b]]); // Атаковать
...
begin
  ...
  begin
    ...
    TextOut(82*CharX, 19*CharY, 'НУЖНО    :'+IntToStr(ExpToNxtLvl-exp));
    Font.Color := cBROWN;
    ...
    case pc.status[stHUNGRY] of
      ...
      begin
        Font.Color := cGREEN;
        TextOut(82*CharX, 27*CharY, 'Переел...');
      end;
      0..450    :
      begin
        Font.Color := cGRAY;
        TextOut(82*CharX, 27*CharY, 'Сытый');
      end;
      451..750  :
      begin
        Font.Color := cYELLOW;
        TextOut(82*CharX, 27*CharY, 'Проголодался');
      end;
      751..1200  :
      begin
        Font.Color := cLIGHTRED;
        TextOut(82*CharX, 27*CharY, 'Голоден!');
      end;
      1201..1500 :
      begin
        Font.Color := cRED;
        TextOut(82*CharX, 27*CharY, 'Умираю от голода!');
      end;
    end;
    case pc.status[stDRUNK] of
      ...
      begin
        Font.Color := cYELLOW;
        TextOut(82*CharX, 28*CharY, 'Пьяненький');
      end;
      501..800:
      begin
        Font.Color := cLIGHTRED;
        TextOut(82*CharX, 28*CharY, 'Пьяный! Ик!');
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    case PickUp(TempItem, TRUE) of
      0 :
      begin
        AddMsg('Теперь ты используешь '+ItemsData[pc.eq[cell].id].name3+', а '+ItemsData[TempItem.id].name3+' ты убрал в инвентарь.');
      end;
      2 : // Нет места
