...
begin
  ...
  begin
    ...
    TextOut(3*CharX,19*CharY, '\      - Показать или убрать линии здоровья у видимых монстров');
    TextOut(3*CharX,20*CharY, 'TAB    - Изменить тактику боя');

    TextOut(3*CharX,22*CharY, 'F1     - Помощь (эта страничка)');
    TextOut(3*CharX,23*CharY, 'F2     - Сохранить игру и выйти          {Пока не работает}');
    TextOut(3*CharX,24*CharY, 'F5     - Сделать скриншот');

    Font.Color := cGRAY;
    TextOut(3*CharX,26*CharY, 'Команды не чувствительны к регистру и языку.');
    TextOut(3*CharX,27*CharY, 'Двигаться по диагонали так же можно зажав ALT + стрелки.');
    Font.Color := cLIGHTGRAY;
