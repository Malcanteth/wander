      ...
      case id of
        ...
        begin
          case pc.quest[1] of
            ...
            begin
              w := FALSE;
              AddMsg('Ты представил'+HeroHS(2)+' '+MonstersData[id].name3+'.');
              More;
            ...
            begin
              case Random(3)+1 of
                1 : s := s + '"Ну как? Ты еще не исследовал'+HeroHS(1)+' хранилище? Как жаль!"';
            ...
            begin
              w := FALSE;
              AddMsg('Ты рассказал'+HeroHS(1)+' '+MonstersData[id].name3+' о своем приключении в хранилище.');
              More;
            ...
            begin
              ...
              More;
              AddMsg('"Ты не представляешь, как я тебе благодарен! Ты избавил'+HeroHS(1)+' нас от этого кошмара!"');
              More;
              AddMsg('"Вот, возьми эти деньги, в следующих версиях я может быть дам тебе что-то более весомое :)"');
              More;
              AddMsg('Ты взял'+HeroHS(1)+' золотые монеты и положил'+HeroHS(1)+' их в карман.');
        ...
        begin
          w := False;
          if (Ask(MonstersData[id].name1 + ' говорит: "Могу предложить бутылёк свежего пивасика всего за 15 золотых, хочешь?" [(Y/n)]')) = 'Y' then
          begin
        ...
        begin
          ...
          begin
            if (Ask(MonstersData[id].name1 + ' говорит: "Хочешь я подлечу тебя?" [(Y/n)]')) = 'Y' then
            begin
        ...
        begin
          w := False;
          if (Ask(MonstersData[id].name1 + ' говорит: "Хочешь купить кусок отличного свежего мяса всего за 15 золотых?" [(Y/n)]')) ='Y' then
          begin
            if pc.FindCoins = 0 then
              AddMsg('К сожалению, у тебя совсем нет денег.') else
              if pc.inv[pc.FindCoins].amount < 15 then
...
begin
  ...
  begin
      ...
      begin
        ...
        if (Victim.eq[7].id > 0) and (Random(Victim.dex)+1 = 1) then
          ...
          begin
            ...
            // Если контратака - то уменьшить дамаг (в 1,1 - 2 раза)
            if CA = 1 then
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          AddMsg('[Ты выполнил'+HeroHS(1)+' квест!!!]');
                          pc.quest[1] := 2;
                          More;
                        end;
                      end;
    ...
    begin
      if Ask('Точно напасть на '+MonstersData[Victim.id].name2+'? [(Y/n)]') = 'Y' then
      begin
        AddMsg('Ты неожиданно напал'+HeroHS(1)+' на '+MonstersData[Victim.id].name2+'!');
        if Victim.id = mdBREAKMT then
        begin
          More;
          AddMsg('<Ты почувствовал'+HeroHS(1)+', что пол под твоими ногами развергся...>');
          More;
          ...
          begin
            ...
            begin
              ...
              More;
              AddMsg('Что ты наделал'+HeroHS(1)+'! Теперь вся деревня против тебя!');
              More;
              Fight(Victim, 0);
            end;
          end;
      end else
        AddMsg('Ты немного подумал'+HeroHS(1)+' и решил'+HeroHS(1)+' этого не делать.');
    end;
  end else
...
begin
  ...
  begin
    if Ask('Точно отдать '+ItemName(GivenItem, 1, TRUE)+' '+MonstersData[Victim.id].name3+'? [(Y/n)]') = 'Y' then
    begin
      // 0-успешно,1-ничего нет,2-нет места,3-перегружен
      case Victim.PickUp(GivenItem, FALSE) of
        0 :
        begin // Успешно отдал
          AddMsg(MonstersData[id].name1+' отдал'+HeSheIt(id,1)+' '+MonstersData[Victim.id].name3+' '+ItemName(GivenItem, 1, TRUE)+'.');
      ...
      AddMsg('Немного подумав, ты решил'+HeroHS(1)+' этого не делать.');
  end else
